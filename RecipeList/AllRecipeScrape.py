...
try:
    ...
    main_div = BeautifulSoup(str(soup.find_all('div', {"id":"insideScroll"})), features="html5lib")
    main_links = list()
    ...
    for class_link in main_links:
        ...
        # Getting Class Name
        class_name = BeautifulSoup(str(soup.find('span', {"class": ["title-section__text", "title"]})), features="html5lib").get_text()
        print('###' + class_name)
        worksheet.merge_range(current_row, first_col, current_row, last_col, class_name, class_merge_format)
        addone()
        worksheet.merge_range(current_row, first_col, current_row, last_col, '', class_merge_format)
        addone()

        # Getting recipe links from main page
        main_div = BeautifulSoup(str(soup.find_all('h3', {"class": ["fixed-recipe-card__h3"]})), features="html5lib")
        for anc in main_div.find_all('a'):
        ...
        for link in class_rec_links:
            ...
            soup = BeautifulSoup(page, features="html5lib")
            name = BeautifulSoup(str(soup.find('h1', {"id": ["recipe-main-content"]})), features="html5lib").get_text()
            print(name)
            ...
            procedure = list()
            proc = BeautifulSoup(str(soup.find('ol', {"class": ["list-numbers", "recipe-directions__list"], "itemprop": ["recipeInstructions"]})), features="html5lib")
            ...
            if len(times) != 0:
                ...
                try:
                    worksheet.write(current_row, prep_t_col, times[0], recipe_format)  # Preparation Time
                    worksheet.write(current_row, cook_t_col, times[1], recipe_format)  # Cook Time
                    worksheet.write(current_row, ready_t_col, times[2], recipe_format)  # Ready to cook Time
                except IndexError:
                    continue


            # Get Nutrition of Recipe
            nutri = BeautifulSoup(str(soup.find('div', {"class": "nutrition-summary-facts"})), features="html5lib")
            nutrition = ' '.join(nutri.get_text().split()).replace('Full nutrition', '')
            # print(nutrition)
            ...
            # Add name to excel
            worksheet.merge_range(start_row, first_col, max_row, first_col, name, merge_format)
            current_row = max_row
            addone()
